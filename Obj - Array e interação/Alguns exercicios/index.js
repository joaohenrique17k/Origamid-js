...
randomNumbers.map((item) => (item % 2 == 1 ? ex1.push(item) : ''));
...
const ala = numbers.map((item) => item * 2);
console.log(ala);
...
const novinho = tarantinoMovies.filter((item) => {
  return item.release < 2000;
});
console.log(novinho);
/*
05
...
const nomeseries = tvShows.map((item) => {
  return item.name;
});
...
cart.map(({ name, price }) => console.log(` -  ${name} Preço: ${price}`));
